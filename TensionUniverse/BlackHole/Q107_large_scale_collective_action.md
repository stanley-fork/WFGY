...
Q107 is not a single conjecture with a yes or no answer. It is a structured multi part open problem about mechanism design, incentives, and large scale behavior.
...
* Q106 (BH_COMPLEX_NETWORK_ROBUST_L3_106)
  Reason: Supplies multi layer network robustness tools that Q107 reuses to model how participation and norms propagate.
...
for some finite integer `k`, which holds all continuous valued parameters such as payoffs, beliefs, and network weights.
...
Higher values represent higher average sacrifices per contributing agent.
...
An element of `E_CA` is a mapping:
...
We fix a single `RefIncentive_CA` in `E_CA` once for Q107 and do not adjust it based on experimental outcomes.
...
* `EffectiveIncentive_CA(m)` is the average of `Incentive_CA(m; i)` across all types weighted by their population share.
...
We define a reference expectation mapping:

```txt
RefBelief_CA: M_CA -> [0, 1]
```

that encodes, for each configuration, the participation fraction that a benchmark rational expectations model would predict, given `RefIncentive_CA` and `Network_CA(m)`. We only use the following properties:
...
The pair `(w_incentive, w_expectation)` is chosen once for Q107 and held constant across all states and experiments.
...
and `DeltaS_CA(m) = 0` only if both incentive and expectation mismatches vanish.

### 3.4 Effective tension tensor and invariants

We adopt an effective tension tensor:

```txt
T_ij_CA(m) = S_i_CA(m) * C_j_CA(m) * DeltaS_CA(m) * lambda_CA(m) * kappa_CA
```
...
* `DeltaS_CA(m)` is the combined mismatch defined above.
...
* `kappa_CA` is a fixed coupling constant setting the scale for incentive_tension in Q107.
...
Interpreted as a measure of how surprising the observed participation is, relative to the reference model.
...
Both are nonnegative scalars that can be compared across configurations.
...
We restrict all Q107 analysis to the regular subset:

```txt
M_CA_reg = M_CA \ S_sing_CA
```
...
This block states how Q107 is characterized as an incentive_tension problem at the effective layer.

### 4.1 Core tension functional

We define the collective action tension functional:

```txt
Tension_CA(m) = G_CA(DeltaS_incentive(m), DeltaS_expectation(m))
```

where `G_CA` is a fixed nonnegative function. A simple choice is:

```txt
Tension_CA(m) = a_CA * DeltaS_incentive(m)
...
These constants are chosen once for Q107 and remain fixed. We require:

* `Tension_CA(m) >= 0` for every `m` in `M_CA_reg`
* `Tension_CA(m)` is small when both mismatches are small
* `Tension_CA(m)` grows when either mismatch grows while the other is held fixed
...
At the effective layer, Q107 can be framed as the following principle:
...
* participation fraction `p_part(m_good)` above a threshold for successful collective action
* non trivial participation costs `C_indiv(m_good)` above a minimal cost level

such that:

```txt
Tension_CA(m_good) <= epsilon_CA
```

for a small threshold `epsilon_CA` that does not grow without bound when we refine our description of incentives, beliefs, and networks within an admissible encoding class.

Informally, world class collective action corresponds to states where many agents participate despite real costs, and where the effective tension between incentives, beliefs, and observed participation can be made consistently small.

### 4.3 Fragile collective action as persistent high tension

Failure to sustain large scale collective action corresponds, at the effective layer, to the following pattern:

For configurations:

```txt
m_fragile in M_CA_reg
```
...
```txt
Tension_CA(m_fragile) >= delta_CA
```

for some strictly positive `delta_CA` that cannot be reduced arbitrarily by refining the encoding within the fixed class `E_CA` and fixed parameters `a_CA`, `b_CA`, `w_incentive`, and `w_expectation`.
...
We describe two counterfactual worlds at the effective layer. They are not generative models. They are patterns of observables and tension values across configurations.
...
for some thresholds `C_min` and `p_thresh` that represent meaningful cost and participation.
...
```txt
Tension_CA(m_T) <= epsilon_CA
```

for some small `epsilon_CA`. When we refine the representation of incentives and beliefs within the same encoding class, `Tension_CA(m_T)` remains within a narrow band rather than drifting upward.

3. Shock resilience

When exogenous shocks change payoff parameters or network structure moderately, there exist sequences of states `m_T_prime` in `M_CA_reg` where:

* participation fraction remains above `p_thresh` after adaptation
* tension `Tension_CA(m_T_prime)` remains in the low band after a transient adjustment period
...
```txt
Tension_CA(m_F) >= delta_CA
```

for some positive `delta_CA` that remains bounded away from zero when we refine the encoding within the fixed class. Both:
...
The World T and World F descriptions do not specify how states in `M_CA` are generated from micro level data. They only assert that if we can construct effective descriptions of worlds with robust or fragile collective action, then the pattern of observables and `Tension_CA` values will differ as above.
...
2. Using the fixed `RefIncentive_CA` and `RefBelief_CA` chosen for Q107, compute:
...
3. Classify each treatment as:
   ...
   * empirically high cooperation (observed `p_part(m_lab)` above threshold)
   * empirically low cooperation (observed `p_part(m_lab)` below threshold)

4. Compare `Tension_CA(m_lab)` distributions between these two empirical groups.

*Metrics:*

* Mean and median `Tension_CA` for high cooperation versus low cooperation treatments.
* Fraction of high cooperation treatments with `Tension_CA` below a pre declared low tension band.
* Fraction of low cooperation treatments with `Tension_CA` above a pre declared high tension band.
* Stability of these patterns when the encoding is applied to new datasets or additional treatments.

*Falsification conditions:*

* If a large majority of high cooperation treatments are assigned `Tension_CA` values in the high tension band while many low cooperation treatments fall in the low tension band, the current encoding of `DeltaS_incentive`, `DeltaS_expectation`, or `Tension_CA` is considered falsified at the effective layer.
* If moderate changes in experimental details that should not alter the logic of cooperation lead to extreme and inconsistent changes in `Tension_CA(m_lab)` without clear theoretical reason, the encoding is considered unstable and rejected.
...
*Boundary note:*
Falsifying TU encoding != solving canonical statement. This experiment can reject specific Q107 encodings but cannot by itself provide a general theory of large scale collective action.
...
1. For each case, assign `m_field` and compute:
   ...
   using the same fixed reference maps and weights as in Experiment 1.
...
* Distribution of `Tension_CA(m_field)` in successful versus failed cases.
* Correlation between `Tension_CA(m_field)` and independent assessments of governance quality or resource outcomes.
* Robustness of results under different reasonable choices of thresholds for success and failure.

*Falsification conditions:*

* If a large proportion of clearly successful long lived commons cases produce `Tension_CA(m_field)` in the high tension band, while many failed cases produce low tension values, the current Q107 encoding is considered misaligned with empirical evidence and must be revised.
* If including additional institutional detail in `m_field` consistently drives `Tension_CA(m_field)` away from low tension for successful cases, instead of stabilizing it, the encoding is considered structurally flawed.
...
*Boundary note:*
Falsifying TU encoding != solving canonical statement. The experiment tests one effective encoding against empirical patterns but does not fully resolve the mechanisms of collective action.

---

## 7. AI and WFGY engineering spec

This block specifies how Q107 can be used as an engineering module for AI systems in the WFGY framework, at the effective layer.
...
3. `signal_CA_tension_total`
   ...
   * Definition: equal to `Tension_CA(m)` when the model is in a socio technical collective action context.
...
* What to log
  ...
  * Any auxiliary estimates of `Tension_CA(m)` produced by the `CollectiveActionHead_CA`.
...
3. ComponentName: `CounterfactualCollectiveWorlds_CA`
   ...
   * Preconditions:
     ...
     * Scenario class allows construction of effective summaries in `M_CA_reg`.
...
* `E_level: E1`
  ...
  * An effective encoding has been specified:
    ...
    * combined tension `Tension_CA`

  * An admissible encoding class `E_CA` and fixed weights have been defined to prevent free post hoc tuning.
...
1. Implement a simple tool that, given summarized data from laboratory or field studies, constructs states in `M_CA_reg` and computes:
...
2. Apply this tool to at least one publicly available dataset from collective action experiments and one from commons governance, and publish the resulting tension profiles.
...
1. Provide detailed case studies of:
   ...
   * one fragile world F like system
   ...
   and explicitly map which mechanisms act on `DeltaS_incentive` and `DeltaS_expectation`.

2. Use Q107 components in at least one downstream problem description, such as Q108 or Q110, with explicit references to shared components.
...
We combine these into a single number called `Tension_CA`.
