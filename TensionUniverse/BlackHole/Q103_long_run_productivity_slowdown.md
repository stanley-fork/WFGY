...
1. It is the primary socio technical instance of an incentive_tension problem, where:
...
1. OECD, “OECD Compendium of Productivity Indicators”, OECD Publishing, multiple editions. Official statistical overview of productivity measures and slowdown patterns in advanced economies.
2. US Bureau of Labor Statistics, “Multifactor Productivity Trends” and related technical notes, various years. Official data and methodology for labor and multifactor productivity in the United States.
3. Robert J. Gordon, “The Rise and Fall of American Growth”, Princeton University Press, 2016. Structural narrative of long run US productivity and headwinds explanation of slowdown.
4. Lawrence H. Summers, “U.S. Economic Prospects: Secular Stagnation, Hysteresis, and the Zero Lower Bound”, Business Economics, 2014, and related work on secular stagnation. Formalization of the idea of persistently low growth and real interest rates.
5. OECD, “The Future of Productivity”, OECD Publishing, 2015. Analysis of productivity slowdown, firm heterogeneity, and diffusion of innovation in advanced economies.
...
All content in this block lives at the effective layer. We describe state spaces, observables, mismatch functionals, tension tensors, and singular sets. We do not describe any hidden generative rules or procedures that build internal fields from raw data.
...
To prevent arbitrary adjustment, we restrict `g_potential` to a fixed admissible encoding class described in 3.4.
...
These fairness conditions define `E_prod`, the admissible class of productivity encodings used in Q103. All tension statements in later blocks are implicitly quantified over this class.
...
- `DeltaS_total(m)` is a combined mismatch:
  ...
  with fixed nonnegative weights `w_prod`, `w_innov`, and `w_const` chosen once for all states in the encoding.
...
- `kappa_prod` is a constant that sets the overall scale of productivity tension in this problem.

Weights are fixed ex ante and cannot be tuned per state. They are part of the encoding specification and must be documented in any concrete implementation.
...
This is a restrict domain treatment of singularities. No attempt is made to regularize tension values inside `S_sing`. Instead those states are explicitly marked as out of scope for Q103 tension evaluation.
...
- Rejecting the encoding means that this particular choice of potential, constraints, and weights is not a valid Q103 effective layer representation.
...
Falsifying TU encoding != solving canonical statement. This experiment can reject specific productivity tension encodings, but it does not explain the slowdown or select a unique economic theory.
...
- If the only way to keep `DeltaS_total` low is to select different parameter values for each window in a way that effectively depends on `g_actual`, violating the no outcome dependent tuning rule, then the encoding is rejected for violating `E_prod`.
...
Falsifying TU encoding != solving canonical statement. This experiment can show that some constraint based explanations are inadequate or unstable, but it does not determine the correct structural explanation for the slowdown.
...
This block describes Q103’s current verification status and future steps.
...
- E_level: E1
  ...
  - A coherent effective layer encoding has been specified:
    ...
    - admissible encoding class `E_prod`,
    - tension tensor structure.
  - Experiments have been outlined with explicit falsification conditions for encodings, but no actual implementation is assumed here.
...
2. Run both experiments in Block 6 with public data and publish:
   - raw data used,
...
Q103 is therefore a reference node for long run productivity puzzles in the Tension Universe, written so that different theories can be compared without claiming that any of them is already the final answer.
