...
* Output: a scalar summarizing the density and low-order statistics of nontrivial zeros in `R` as encoded in `m`.
...
The value `rho_zero(m_R)` may be a short feature vector, but whenever we speak of norms we use an L2 norm on this fixed finite-dimensional representation.
...
An element of `Enc_RH` is a tuple
...
* `Ref_spec` is selected from a finite library built only from:
  ...
  * and optionally random-matrix-inspired reference ensembles specified in advance.
...
* Both are fixed before any evaluation.
...
Each world is described through observable tension patterns. No hidden construction rules are provided.
...
* Test whether the frozen `Tension_RH` functional and reference classes produce stable, non-cheatable tension profiles on published numerical datasets.
...
* Freeze a specific encoding tuple `Enc` in `Enc_RH`:
  ...
  * fix `w_spec`, `w_arith`, `alpha`, `beta` within locked constraints,
  * fix `ToleranceModel`,
...
* If the encoding yields materially different qualitative conclusions under small, pre-registered parameter changes inside the allowed ranges, it is rejected as non-auditable.
...
2. Q003 (Birch and Swinnerton-Dyer conjecture)
   ...
   * Why it transfers: BSD links L-function spectral behavior to arithmetic invariants, enabling similar World T versus World F patterns.
...
The key anti-cheat idea is that the reference baselines, weights, coefficients, coupling rule, and resolution ladder are frozen before evaluation and audited. You cannot look at results and then pick a new baseline that makes tension small.
...
* It makes the RH world-versus-non-RH distinction auditable at the effective layer.
