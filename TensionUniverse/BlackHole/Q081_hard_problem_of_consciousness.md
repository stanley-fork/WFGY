...
* There are physical processes in the nervous system that can be described in objective terms, such as neural firing, network dynamics, and information processing.
...
* The hard problem is not simply to correlate these two domains, but to explain why and in what structural sense particular neural processes give rise to particular patterns of subjective experience, rather than to none or to different ones.
...
* There is significant progress on neural correlates of consciousness, global workspace style theories, integrated information theories, and predictive processing frameworks.
...
* a genuine scientific problem that will be solved by deeper theory and richer data,
* a sign that current conceptual frameworks are inadequate,
* or a pseudo problem that will eventually be dissolved by reanalysis of our concepts of experience and physical reality.
...
5. Standard encyclopedia entry on "Hard problem of consciousness" or "Unsolved problems in neuroscience", covering the distinction between easy and hard problems.
...
We do not describe any hidden generative rules, and we do not describe how to construct internal TU fields from raw neural or behavioral data.
...
* or how raw data are transformed into these summaries.

We only assume that for the states in `M` the observables defined below are well defined.
...
* It does not attempt to capture the full richness of experience, only a coarse structured summary.
...
* It includes such information as stimulus class, task instructions, attention conditions, pharmacological state, and so on.
...
To prevent trivial reductions of tension by adjusting the encoding after seeing the data, we restrict attention to an admissible encoding class defined by:
...
* A finite set of neural invariants such as:
  ...
  * band limited oscillation types,
...
* Refinement rules:
  ...
  * For each `k` there is a corresponding encoding class with its own `Library_experience(k)` and `Library_neural_invariants(k)`, but all are chosen according to a pre specified protocol.
  * Refinement is not allowed to re define the conceptual meaning of invariants or categories, only to sharpen them.

All tension measures and invariants defined in this document are implicitly indexed by `k`, even when this is not written explicitly.
...
We then restrict RH style tension analysis for Q081 to the regular domain:
...
Intuitively, no matter how clever the mapping rules or how rich the invariant libraries, certain episodes resist a simple structural alignment between neural patterns and phenomenal structure, and the residual mismatch cannot be compressed away.
...
1. Stable mapping across contexts
   ...
   * For most regular episodes `m_T(k)` in these contexts, `DeltaS_neuro_phen(m_T(k))` is small and does not grow unbounded with refinement.
...
4. Global tension profile
   ...
   * High tension cases exist but can be interpreted as data limitations, noise, or known model misspecifications.
...
4. Global tension profile
   ...
   * This region is robust to changes in tasks, stimuli, and organisms, suggesting an inherent limitation of the encoding approach.
...
* and to provide templates for testing AI systems that attempt to represent or simulate conscious experience.

We keep all statements at the level of observables and tension functionals, not at the level of metaphysical commitments about consciousness.
...
These experiments do not solve the hard problem. They only test whether the chosen encoding behaves in a way that is consistent with the qualitative distinctions between World T and World F.
...
3. On the test set:
   ...
   * compute `DeltaS_neuro_phen(m_test(k))`,
   * compute `DeltaS_task_generalization(m_test(k))`,
   * compute `Tension_CONS(m_test(k))` for each episode.
...
* If small changes in encoding parameters produce arbitrarily large changes in the tension distribution on the same dataset without clear justification, the encoding is considered unstable and rejected.

*Semantics implementation note:*

In this experiment, neural observables are treated as continuous valued descriptors derived from continuous time signals, while reports and experience categories are treated as discrete labels. The overall treatment is hybrid, consistent with the metadata, but no differential operators are introduced.
...
* Difference in mean and variance of `Tension_CONS` between baseline and dissociation episodes.
...
Again, neural descriptors are continuous and reports are discrete, with the hybrid representation used only to define finite summaries. No assumptions are made about the underlying metaphysics of consciousness.
...
4. `signal_counterfactual_separation`
   ...
   * Use: encourages explicit tracking of assumptions and prevents mixing incompatible worlds.
...
* What to log
  ...
  * This allows external review of how the encoding behaves without exposing any deep TU generative rules.
...
3. Q121 (AI alignment problem)
   ...
   * Why it transfers:
     ...
     * Alignment can be framed as reducing tension between an AI system's internal states and human reported values or experiences.
...
* E_level: E1
  ...
  * The effective encoding for Q081 has:
    ...
    * an admissible encoding class with finite libraries, fixed mapping rules, and refinement parameter.
...
* and a set of tools that can also be used for related problems in neuroscience, philosophy, and AI.
